...
async function loadWorkbookFromShareLink(shareUrl, accessToken, tabNames) {
  ...
  for (const name of tabNames) {
    ...
    // Same for columns: usedRange starts at the first USED column, so a tab
    // whose column A is empty (e.g. "3. New Listings") comes back with
    // columnIndex 1 and every row shifted one to the left. xlsx-lite kept
    // absolute column positions (A = index 0), and generate.js reads fixed
    // indices - so left-pad to restore them.
    const startCol = range.columnIndex || 0;
    const values = range.values || [];
    cache[name] = values.map((row, idx) => {
      // Graph returns native JSON types (numbers like date serials 45643 or
      // long IDs, booleans from checkbox cells) while xlsx-lite.js always
      // returned the raw cell text as a string - and all of generate.js's
      // row logic (.trim(), regex tests, safeNumber, ...) was written against
      // strings. Normalising here keeps the "same interface" promise above
      // true for cell VALUE types too, not just for the method names.
      // Booleans become "1"/"0", exactly what xlsx-lite read from t="b" cells.
      const arr = new Array(startCol).fill("").concat(
        row.map((v) => {
